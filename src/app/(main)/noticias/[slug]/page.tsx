  ...
  if (error || !post) {
    return (
      <div className="flex flex-col justify-center items-center min-h-[calc(100vh-15rem)] space-y-6">
        ...
        <p className="text-gray-600 text-lg">Não foi possível carregar a notícia.</p>
        <Link href="/noticias">
          <Button variant="outline" className="flex items-center mb-8 cursor-pointer hover:bg-primary">
    ...
    <article className="container mx-auto max-w-4xl py-12 px-4">
      ...
      {post.coverImage && (
        <div className="relative w-full h-64 md:h-96 rounded-lg overflow-hidden mb-8 shadow-lg">
          <Image src={post.coverImage} alt={post.title} layout="fill" className="object-cover" priority />
        </div>
      )}
